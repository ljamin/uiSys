...
<div id="outside-title" style="text-align: center;font-size:26px;">
    <span style="text-align: center">消息通知</span>
...
<table class="layui-hide" id="showMyNotices" lay-filter="showMyNotices" style="margin-left: 2px"></table>
<script id="status" type="text/html">
    {{# if(d.readstatus==0){}}
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="">未读</a>
    {{#  } }}
    {{# if(d.readstatus==1){}}
    <a class="layui-btn layui-btn-xs" lay-event="">已读</a>
    {{#  } }}
</script>
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#showMyNotices'
            ,page: true
            ,url:'./showMyNotices.action'      //获取数据的action
            ,cols: [[
                {field:'id', title:'ID', width:200, fixed: 'left', unresize: true, sort: true}
                ,{field:'content', title:'内容', width:180}
                ,{field:'createtime', title:'创建时间', width:200}
                ,{field:'fromUserId', title:'发起人ID', width:200,templet: '<div><a href="javascript:void(0)" ' +
        ...
        'onclick="detail(\'{{d.toNewsId}}\',\'{{d.id}}\')" class="layui-table-link">{{d.toNewsId}}</a></div>'}
                ,{field:'status', title:'状态', width:220,toolbar:'#status'}
            ]]
            ,done:function(res){
            }
            ,id:'reload'
    ...
    function detail(newsId,remindId){
        $(function () {
            $.ajax({
                type: "get",
                url: "getNewsDetail.action?id=" + newsId+"&remindId="+remindId,
                dataType: 'json',
                success: function (result) {
